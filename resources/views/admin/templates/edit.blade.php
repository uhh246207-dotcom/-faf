...
<div class="mb-6">
    ...
    <h1 class="text-2xl font-bold text-gray-900 mt-2">Sua Template: {{ $template->name }}</h1>
</div>
    ...
    <form method="POST" action="{{ route('admin.templates.update', $template) }}" enctype="multipart/form-data" class="space-y-6">
        ...
        <div>
            ...
            <textarea name="description" id="description" rows="3"
                      class="w-full px-4 py-2.5 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 @error('description') border-red-500 @enderror">{{ old('description', $template->description) }}</textarea>
            @error('description')
                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
            @enderror
        </div>
        ...
        <div>
            ...
            <input type="number" name="token_price" id="token_price" value="{{ old('token_price', $template->token_price) }}" min="0" required
        ...
        <div>
            ...
            @if ($template->psd_path)
                <p class="text-xs text-gray-500 mb-1">File hien tai: {{ basename($template->psd_path) }}</p>
            @endif
            ...
            <p class="mt-1 text-xs text-gray-500">De trong neu khong muon doi file PSD</p>
            @error('psd_file')
                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
            @enderror
        </div>
        ...
        <div>
            ...
            @if ($template->preview_path)
                <div class="mb-2">
                    <img src="{{ Storage::url($template->preview_path) }}" alt="Preview" class="w-32 h-auto rounded-lg border border-gray-200">
                </div>
            @endif
            ...
            <p class="mt-1 text-xs text-gray-500">De trong neu khong muon doi anh preview</p>
            @error('preview_file')
                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
            @enderror
        </div>
