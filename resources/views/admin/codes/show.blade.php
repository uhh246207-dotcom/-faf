...
@section('title', 'Chi tiet code - ' . $code->code)
...
<div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6 max-w-2xl">
    <dl class="space-y-4">
        <div class="flex justify-between py-2 border-b border-gray-100">
            ...
            <dd class="text-sm font-mono font-bold text-gray-900">{{ $code->code }}</dd>
        </div>
        <div class="flex justify-between py-2 border-b border-gray-100">
            <dt class="text-sm font-medium text-gray-500">So token</dt>
            <dd class="text-sm font-medium text-indigo-600">{{ $code->token_amount }} tokens</dd>
        </div>
        <div class="flex justify-between py-2 border-b border-gray-100">
        ...
        <div class="flex justify-between py-2 border-b border-gray-100">
            ...
            <dd class="text-sm text-gray-900">{{ $code->createdBy?->name ?? 'N/A' }}</dd>
        </div>
        <div class="flex justify-between py-2 border-b border-gray-100">
            <dt class="text-sm font-medium text-gray-500">Ngay tao</dt>
            <dd class="text-sm text-gray-900">{{ $code->created_at->format('d/m/Y H:i:s') }}</dd>
        </div>
        @if ($code->usedBy)
        <div class="flex justify-between py-2 border-b border-gray-100">
            <dt class="text-sm font-medium text-gray-500">Nguoi su dung</dt>
            <dd class="text-sm text-gray-900">{{ $code->usedBy->name }} ({{ $code->usedBy->email }})</dd>
        </div>
        @endif
        @if ($code->used_at)
        <div class="flex justify-between py-2 border-b border-gray-100">
            <dt class="text-sm font-medium text-gray-500">Thoi gian su dung</dt>
            <dd class="text-sm text-gray-900">{{ $code->used_at->format('d/m/Y H:i:s') }}</dd>
        </div>
        @endif
        @if ($code->note)
        <div class="flex justify-between py-2 border-b border-gray-100">
            <dt class="text-sm font-medium text-gray-500">Ghi chu</dt>
            <dd class="text-sm text-gray-900">{{ $code->note }}</dd>
        </div>
        @endif
    </dl>
    ...
    <div class="mt-6 flex items-center space-x-3">
        @if ($code->status === 'active')
        <form method="POST" action="{{ route('admin.codes.disable', $code) }}"
